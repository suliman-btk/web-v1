...
if ($isEdit) {
    ...
    if (!$found) { set_flash('error', 'Product not found.'); redirect('seller/products.php'); }
    $p = $found;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($p['price']) || (float)$p['price'] <= 0) $errors['price'] = 'Enter a valid price.';
    if ($p['discount_price'] !== '' && (!is_numeric($p['discount_price']) || (float)$p['discount_price'] <= 0))
        $errors['discount_price'] = 'Discount price must be positive.';
    elseif ($p['discount_price'] !== '' && (float)$p['discount_price'] >= (float)$p['price'])
        $errors['discount_price'] = 'Discount must be lower than regular price.';
...
?>
<p><a href="<?= e(url('seller/products.php')) ?>">← Back to Products</a></p>

<div class="panel" style="max-width:720px">
    <?php if (!empty($errors['general'])): ?>
        <div class="flash flash-error"><?= e($errors['general']) ?></div>
    <?php endif; ?>
    <form method="post" action="<?= e(url('seller/product_form.php' . ($isEdit ? '?id=' . $id : ''))) ?>" enctype="multipart/form-data" novalidate>
        <?= csrf_field() ?>
        ...
        <div class="field">
            ...
            <div class="hint">Used only when no file is uploaded. Leave blank for default.</div>
        </div>
        <div class="field">
        ...
        <a href="<?= e(url('seller/products.php')) ?>" class="btn btn-ghost">Cancel</a>
    </form>
</div>
